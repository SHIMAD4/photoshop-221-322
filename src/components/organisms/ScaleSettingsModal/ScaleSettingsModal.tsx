...
const ScaleSettingsModal: FC<Props> = ({
    ...
    const getActual = () => ({
        width:
            unit === 'pixels'
                ? inputWidth
                : Math.round(initialWidth * (inputWidth / 100)),
        height:
            unit === 'pixels'
                ? inputHeight
                : Math.round(initialHeight * (inputHeight / 100)),
    })
            ...
            <DialogContent>
                ...
                <FormControl fullWidth margin='normal'>
                    ...
                    <Select
                        ...
                        value={interp}
                        onChange={(e) =>
                            setInterp(e.target.value as Interpolation)
                        }
                    >
                        <MenuItem value='nearest'>
                            <Tooltip title='Простой и быстрый алгоритм, но может приводить к появлению ступенчатых артефактов.'>
                                <span>Ближайший сосед</span>
                            </Tooltip>
                        </MenuItem>
                        <MenuItem value='bilinear'>
                            <Tooltip title='Гладкая интерполяция: обеспечивает высокое качество, но работает медленнее.'>
                                <span>Билинейная</span>
                            </Tooltip>
                        </MenuItem>
                    </Select>
                </FormControl>
