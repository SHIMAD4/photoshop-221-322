...
import { drawAlphaPreviewToCanvas } from "../../../utils/drawAlphaPreviewToCanvas";
import { drawPreviewToCanvas } from "../../../utils/drawPreviewToCanvas";
import "./style.css";
...
const blendModes: { value: BlendMode; label: string; tooltip: string }[] = [
	{
		value: "normal",
		label: "Обычный",
		tooltip: "Обычный режим: верхний слой полностью перекрывает нижний в зависимости от прозрачности.",
	},
	{
		value: "multiply",
		label: "Умножение",
		tooltip: "Умножение: делает изображение темнее, умножая значения цветов слоёв.",
	},
	{
		value: "screen",
		label: "Экран",
		tooltip: "Экран: делает изображение светлее, инвертируя, умножая, затем инвертируя снова.",
	},
	{
		value: "overlay",
		label: "Наложение",
		tooltip: "Наложение: сочетание multiply и screen, усиливающее контраст.",
	},
];
	...
	return (
		<div className="layers_panel">
			...
			{layers.map((layer, index) => (
				<div
					key={index}
					className={`layer-item ${index === activeIndex ? "active-layer" : ""}`}
					onClick={() => setActiveIndex(index)}
				>
					<p>Слой {index + 1} ({layer.type})</p>
					<canvas
			...
			<hr />
			<p><strong>Активный слой</strong></p>
			...
			<p><strong>Режим наложения</strong></p>
			...
			<p><strong>Альфа-канал:</strong></p>
			{imageData.hasAlpha && (
				<div className="alpha-buttons">
					<Tooltip title={imageData.alphaHidden ? "Показать альфу" : "Скрыть альфу"}>
						<button onClick={() => onAlphaHide?.(!imageData.alphaHidden)}>
							<RemoveRedEyeIcon style={{ color:'white' }}/>
						</button>
					</Tooltip>
					<Tooltip title="Удалить альфу">
						<button onClick={onAlphaRemove} disabled={!imageData.hasAlpha}>
			...
			<p><strong>Слой:</strong></p>
			<div className="layer-buttons">
				<Tooltip title={imageData.visible === false ? "Показать слой" : "Скрыть слой"}>
					<button onClick={() => onToggleVisibility?.(!(imageData.visible ?? true))}>
						{imageData.visible === false ? <VisibilityIcon style={{ color:'white' }}/> : <VisibilityOffIcon style={{ color:'white' }}/>}
					</button>
				</Tooltip>
				<Tooltip title="Удалить слой">
					<button onClick={onDeleteLayer}><DeleteIcon style={{ color:'white' }}/></button>
				</Tooltip>
				<Tooltip title="Переместить вверх">
					<button onClick={onMoveLayerUp}><ArrowUpwardIcon style={{ color:'white' }}/></button>
				</Tooltip>
				<Tooltip title="Переместить вниз">
					<button onClick={onMoveLayerDown}><ArrowDownwardIcon style={{ color:'white' }}/></button>
				</Tooltip>
			</div>
